...
import 'package:ffi/ffi.dart'; // For Utf8 and calloc
...
final DynamicLibrary lib = Platform.isWindows
    ? DynamicLibrary.open("desktop_drop_plugin.dll")
    : DynamicLibrary.process();

typedef GetPngFunc = Void Function(Pointer<Utf8> filePath, Pointer<Utf8> pngPath);
typedef GetPng = void Function(Pointer<Utf8> filePath, Pointer<Utf8> pngPath);

final getPng = lib
    .lookup<NativeFunction<GetPngFunc>>("getPng")
    .asFunction<GetPng>();

void getPngByPath(String filePath,String pngPath){
  final filePathC = filePath.toNativeUtf8();
  final pngPathC = pngPath.toNativeUtf8();

  getPng(filePathC, pngPathC);

  calloc.free(filePathC);
  calloc.free(pngPathC);
}